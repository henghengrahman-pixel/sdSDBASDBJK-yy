...
import { TelegramClient, telegramLimiterMetrics } from './telegram.js';
...
const learningWorker=createLearningWorker({
  ...
  withLock:withLearningAdvisoryLock,isBusy:()=>{const p=pollerStatus();return Boolean(p.running||p.workers?.activeWorkers>0||Number(p.lastResult?.queueDepth||0)>Math.max(50,config.workerBatchSize*4));}
});
...
function isRequesterNotUserError(err){return Number(err?.status)===403&&/requester\s+is\s+not\s+(?:a\s+)?user\s+of\s+the\s+chat/i.test(String(err?.message||err?.data?.error?.message||''));}
async function assertConversationSendable(chatId){
  const lifecycle=await getConversationLifecycleState(chatId);
  if(!lifecycle||lifecycle.visible_in_inbox===false||['closed','archived'].includes(String(lifecycle.status||'').toLowerCase())){const e=new Error('LIVECHAT_SEND_BLOCKED_CLOSED_OR_HIDDEN');e.status=409;throw e;}
  return lifecycle;
}
async function handleRequesterNotUser(chatId,error){
  if(!isRequesterNotUserError(error))return false;
  incLiveChatMetric('send403RequesterNotUser');
  let refresh=null;
  try{const chat=await lc.getChat(chatId,{id:chatId});refresh=lc.chatState(chat||{});if(refresh.isClosed||refresh.isArchived)await closeConversationFromLiveChat(chatId,'SEND_403_PROVIDER_TERMINAL',{...refresh,archived:refresh.isArchived});}catch(refreshError){await logError('livechat_send','LIFECYCLE_REFRESH_AFTER_403_FAILED',refreshError.message,{chatId}).catch(()=>{});}
  await setIntegrationHealth('livechat_send_message',{status:'WARNING',error:String(error?.message||error),meta:{lastAction:'sendMessage',requesterNotUser:true,refresh}}).catch(()=>{});
  return true;
}
...
app.get('/api/conversations',requireAdmin,async(req,res)=>{
  ...
  const r=await pool.query(`SELECT c.chat_id,c.customer_name,c.customer_email,c.status,c.handling_mode,c.takeover_reason,c.takeover_at,c.last_event_at,c.last_member_event_at,c.last_ai_event_at,c.workflow_type,c.workflow_state,c.member_typing,c.member_typing_updated_at,c.created_at,c.updated_at,c.lc_inbox_rank,
    ...
    WHERE c.visible_in_inbox=true AND c.status NOT IN ('closed','archived')
...
app.post('/api/conversations/:id/send',requireAdmin,async(req,res)=>{const text=String(req.body?.text||'').trim();if(!text)return res.status(400).json({ok:false,error:'EMPTY_TEXT'});try{const out=await withChatLock(req.params.id,async()=>{await assertConversationSendable(req.params.id);if(!(await isHumanTakeover(req.params.id))) { const er=new Error('HUMAN_TAKEOVER_REQUIRED'); er.status=409; throw er; } const sent=await lc.sendMessage(req.params.id,text);const eventId=sent?.event_id||sent?.id||null;if(eventId){await insertMessage({chatId:req.params.id,eventId:String(eventId),senderType:'agent',authorId:'admin',text,normalizedText:normalizeText(text),intent:detectIntent(text),createdAt:new Date().toISOString()});await captureHumanReplyLearning({chatId:req.params.id,eventId:String(eventId),responseText:text}).catch(()=>{});}await setIntegrationHealth('livechat_send_message',{status:'OK',meta:{lastAction:'sendMessage'}}).catch(()=>{});return {ok:true,sent};});res.json(out);}catch(e){await handleRequesterNotUser(req.params.id,e);res.status(e.status||502).json({ok:false,error:e.message});}});
app.post('/api/conversations/:id/image',requireAdmin,async(req,res)=>{
  try{
    ...
    const out=await withChatLock(req.params.id,async()=>{
      await assertConversationSendable(req.params.id);
    ...
    res.json(out);
  }catch(e){await handleRequesterNotUser(req.params.id,e);await logError('manual_image','SEND_FAILED',e.message,{chatId:req.params.id});res.status(e.status||502).json({ok:false,error:e.message});}
});
